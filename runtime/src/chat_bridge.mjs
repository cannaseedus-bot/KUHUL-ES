...
//
// Browser / service-worker chat-completion bridge for KUHUL-ES.
//
...
import { KuhulThinkEngine, SemanticPatternReasoner } from './think.mjs';
import { kxmlToMessages, renderForGguf, toJinja } from './kxml_chat.mjs';
...
function toolResponseFromOpenAi(toolMessages) {
  // Merge tool results into the KXML dialog as tool role messages.
...
class ChatBridge {
  ...
  _extractArgs(text, parameters) {
    // Very small heuristic: find a quoted or bracketed payload after the tool name.
  ...
  _generateContent(goal, prompt) {
    // Deterministic echo: summarize the prompt length and the user goal.
...
// Service-worker fetch handler for /v1/chat/completions.
function chatCompletionFetchHandler(event, bridge) {
  const url = new URL(event.request.url);
  if (event.request.method !== 'POST' || url.pathname !== '/v1/chat/completions') {
    return;
  }
  event.respondWith((async () => {
    const body = await event.request.json().catch(() => ({}));
    const response = await bridge.complete(body);
