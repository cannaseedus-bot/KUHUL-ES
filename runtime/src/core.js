// runtime/src/core.js
//
// Isomorphic core runtime for KUHUL-ES.
//
// Shared between Node.js (runtime/src/node.js), browser (runtime/src/browser.js),
// and service worker (sw.js). Contains no platform-specific I/O: no fs, no DOM.
// Platform adapters inject event handlers, output streams, and glyph overrides.
//
// Uses the AST-based RuntimeParser + ExpressionEvaluator so π/τ bindings and
// glyph arguments support full expressions (string concat, Math.*, variable refs).

'use strict';

const { RuntimeParser } = require('./runtime_parser');
const { ExpressionEvaluator } = require('./expression_evaluator');
const { KuhulPhysics } = require('./physics');
const { KuhulThinkEngine } = require('./think');

const DEFAULT_GLYPHS = ['Sek', 'Pop', 'Wo', "Ch'en", 'Yax', 'Xul', 'Noj'];

// Deterministic hash using WebCrypto/Node crypto if available, else a simple
// integer hash for environments without crypto (should not happen in Node/browser).
...
class KUHULRuntimeCore {
  constructor(opts = {}) {
    ...
    this.delayMs = opts.delayMs ?? (typeof window !== 'undefined' ? 16 : 10);
    ...
    // allow platform adapters to override or extend glyph implementations
    if (opts.glyphOverrides) {
      for (const [k, v] of Object.entries(opts.glyphOverrides)) {
        this._glyphImpl[k] = v.bind ? v.bind(this) : v;
      }
    }
  }
  ...
  emit(event, data) {
    ...
    for (const h of handlers) {
      try { h(data); } catch (e) { this._log(`event error: ${e.message}`); }
    }
  }
  ...
  async execute(source) {
    try {
      ...
      this.emit('complete', {
        ...
        thoughtCount: this.thinker.thoughts.length,
      });
  ...
  parseBindings(source) {
    ...
    for (const [name, value] of bindings.π) {
      this.π.set(name, value);
    }
    for (const [name, value] of bindings.τ) {
      this.τ.set(name, value);
      if (!this.τHistory.has(name)) this.τHistory.set(name, []);
    }
    ...
    for (const call of this.glyphQueue) {
      ...
      if (result && result.updateTau) {
        for (const [key, value] of Object.entries(result.updateTau)) {
          if (this.τ.has(key)) {
            this.τHistory.get(key).push({
              frame: this.frame,
              value,
              hash: await hashState({ value }),
            });
      ...
      const stateHash = await hashState({
        frame: this.frame,
        glyph: call.glyph,
        args: call.args,
        result,
        physics: this.physics.history[this.physics.history.length - 1] || null,
      });
  ...
  async _executeSek(operation, ...args) {
    ...
    switch (operation) {
      ...
      case 'set':
        return { updateTau: { [args[0]]: args[1] } };
      case 'think':
        return await this._executeNoj(args[0], args[1] || {});
      default:
        return { operation, args };
    }
  }

  async _executePop(value) {
    this._log(`[Pop] ${value}`);
    this.physics.perceive();
    this.emit('log', `[Pop] ${value}`);
    return { value };
  }

  async _executeWo(operation, ...args) {
    this._log(`[Wo] ${operation}: ${JSON.stringify(args)}`);
    this.physics.represent();
    this.emit('log', `[Wo] ${operation}`);
    return { operation, args };
  }

  async _executeChen(source, ...args) {
    this._log(`[Ch'en] ${source}: ${JSON.stringify(args)}`);
    this.physics.project();
    this.emit('log', `[Ch'en] ${source}`);
    return { source, args, timestamp: Date.now() };
  }

  async _executeYax(condition, value) {
    this._log(`[Yax] Condition: ${condition}, Value: ${value}`);
    this.physics.plan();
  ...
  async _executeXul() {
    this._log('[Xul] Stopping execution');
    this.physics.consolidate();
    this.world.active = false;
    return { stopped: true, physics: this.physics.state() };
  }
  ...
  async loadState(state) {
    ...
    this.world = state.world || { bodies: [], fields: [], active: true };
  }
}

module.exports = { KUHULRuntimeCore, hashState, DEFAULT_GLYPHS };
